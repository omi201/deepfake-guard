"""
Phase 3d: Manual PGD Adversarial Training (VALIDATION FIXED)
"""
import torch
...
print(f"{'='*70}")
print("PHASE 3d: MANUAL PGD ADVERSARIAL TRAINING (FIXED)")
print(f"{'='*70}")
...
print(f"[3/3] Starting training...")
print(f"{'='*70}")

history = {"epochs": [], "clean_acc": [], "robust_acc": []}
...
for epoch in range(10):
    ...
    print(f"\nEpoch {epoch+1}/10 | PGD-{steps} | Mix: {mix_ratio*100:.0f}%")
    ...
    # Validation - FIXED: Enable grads for attack
    print(f"  Validating...")
    model.eval()
    val_clean_correct = 0
    ...
    print(f"  Clean={val_clean_acc:.2%} | Robust={val_robust_acc:.2%}")
    ...
    if val_robust_acc > best_robust_acc:
        ...
        print(f"  💾 BEST: {best_robust_acc:.2%}")
    
    if val_robust_acc >= 0.85:
        print(f"\n🎉 TARGET REACHED!")
        break
    
    scheduler.step(val_robust_acc)

with open("data/models/adversarial_training_history.json", 'w') as f:
    json.dump(history, f, indent=2)

print(f"\n{'='*70}")
print(f"COMPLETE! Best Robust: {best_robust_acc:.2%}")
print(f"{'='*70}")
